...
export default async function RootLayout({ children }: RootLayoutProps) {
  ...
  return (
    <html lang="en" suppressHydrationWarning className="scroll-smooth">
      ...
      <body
        className={`${publicSans.variable} ${commitMono.variable} w-full min-h-[100dvh] overflow-x-hidden antialiased touch-pan-y`}
      >
        {children}
        <div className="group fixed bottom-0 left-1/2 z-50 mb-2 -translate-x-1/2 safe-area-bottom px-[env(safe-area-inset-left)]">
          <ThemeToggle className="translate-y-20 transition-transform delay-150 duration-300 group-hover:translate-y-0 group-focus-within:translate-y-0" />
        </div>
      </body>
    </html>
  );
